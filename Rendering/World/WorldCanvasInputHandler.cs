...
public class WorldCanvasInputHandler : ICanvasInputHandler
{
    ...
    public WorldCanvasInputHandler(WorldCanvas canvas)
    {
        _canvas = canvas;
        _world = canvas.World;
    }
        ...
        if (_sizeTool.SizingObject != null)
        {
            if (_lastMousePosition != null)
            {
                ...
                if (!unit.IsZero())
                {
                    ...
                    _canvas.PointerWorldPosition = worldPos;
                }
            }
        }
    }
    ...
    public bool MouseClick(Point relativePosition, MouseButton button, KeyModifiers modifiers)
    {
        ...
        {
            if (_world.AddingObject != null)
            {
                if (_world.AddingObject is not ISizeableObject)
                {
                    DrawTemplate template = _world.AddingObject.Template;
                    _world.PlaceAdding();
                    // continue adding.
            ...
            else
            {
                ...
                var insides = _world.PointTest(worldPos, null);
                if (insides.Count > 0)
                {
                    ...
                    if (!modifiers.IsModifierPressed(KeyModifiers.Control))
                    {
                        ...
                        {
                            var obj = vm.SelectedObjects[i] as WorldObject;
                ...
                else
                {
                    ...
                    for (int i = vm.SelectedObjects.Count - 1; i >= 0; i--)
                    {
                        (vm.SelectedObjects[i] as WorldObject).IsSelected = false;
                    }
                }
            }
        }
    ...
    public bool MouseDragging(Point relativePosition, Point changed, MouseButton button, KeyModifiers modifiers)
    {
        ...
        if (button == MouseButton.Left)
        {
            ...
            if (_dragMode == DragModes.None)
            {
                ...
                if (_world.AddingObject != null && _world.AddingObject.CreateMode == CreateModes.Sizeable)
                {
                ...
                else
                {
                    ...
                    {
                        if (obj is not WorldObject worldObj) continue;
            ...
            else if (_dragMode == DragModes.Translating)
            {
                ...
                int align = Preferences.Singleton.WorldSettings.AlignSize;
                ...
                if (movement != Vector2.Zero)
                {
                    ...
                    {
                        if (obj is not WorldObject worldObj) continue;
                        worldObj.Position += new Vector2(movement.X, -movement.Y);
                    }
                }
            }
            else if (_dragMode == DragModes.Sizing)
            {
                ISizeableObject sizeable = _world.AddingObject != null ? (_world.AddingObject as ISizeableObject) : vm.SelectedObjects[0] as ISizeableObject;
                if (_sizeTool.SizingObject == null)
                {
                    ...
                    if (_world.AddingObject != null)
                    {
                        ...
                        _world.AddingObject.Position = worldStart;
    ...
    public bool MouseDraggingEnd(Point relativePosition, MouseButton button)
    {
        if (button == MouseButton.Left)
        {
            ...
            {
                ISizeableObject sizeable = _world.AddingObject as ISizeableObject;
    ...
    public bool MouseMoving(Point relativePosition, KeyModifiers modifiers)
    {
        ...
        if (_world.AddingObject != null && _world.AddingObject.CreateMode != CreateModes.Sizeable)
        {
            _world.AddingObject.Position = worldPos;
        }
    ...
    public bool KeyReleased(Key key, KeyModifiers modifiers)
    {
        ...
        else if (key == Key.Back || key == Key.Delete)
        {
            if (_world.AddingObject == null)
            {
                foreach (WorldObject obj in _world.SelectedObjects)
                {
                    obj.IsSelected = false;
        ...
        else if (key == Key.Up || key == Key.Down || key == Key.Left || key == Key.Right)
        {
            ...
            double step = controlPressed ? 1.0 : Preferences.Singleton.WorldSettings.AlignSize;
            Avalonia.Vector unit = UnitVectorFromKey(key);
            WorldObject targetObj = _world.AddingObject;
            if (targetObj == null)
            {
                foreach (WorldObject obj in _world.SelectedObjects)
                {
        ...
        else if (key == Key.OemComma || key == Key.OemPeriod)
        {
            ...
            if (targetObj == null)
            {
                targetObj = _world.SelectedObjects.FirstOrDefault();
            }
            if (targetObj != null)
            {
                if (key == Key.OemComma)
                {
                    var overlapped = _world.RectTest(targetObj.BoundingBox, item => item != targetObj && item.Template.Layer == targetObj.Template.Layer && item.RelativeDepth <= targetObj.RelativeDepth).OrderByDescending(item => item.RelativeDepth);
                ...
                {
                    var overlapped = _world.RectTest(targetObj.BoundingBox, item => item != targetObj && item.Template.Layer == targetObj.Template.Layer && item.RelativeDepth >= targetObj.RelativeDepth).OrderBy(item => item.RelativeDepth);
                    var first = overlapped.FirstOrDefault();
                    if (first != null)
                    {
    ...
    {
        WorkspaceViewModel vm = WorkspaceViewModel.Current;
        WorldObject toAdd = template.OnCreateWorldObject();
        toAdd.Position = CanvasPositionToAlignedWorldPosition(posCanvas);
        _world.ReadyAdd(toAdd);
    }

...
public class WorldCanvas : OpenGLCanvas
{
    ...
    public static readonly StyledProperty<float> ViewScaleProperty = AvaloniaProperty.Register<WorldCanvas, float>(nameof(ViewScale), defaultValue: 1.0f);
    ...
    public static readonly StyledProperty<Avalonia.Vector> PointerWorldPositionProperty = AvaloniaProperty.Register<WorldCanvas, Avalonia.Vector>(nameof(PointerWorldPosition), defaultValue: Avalonia.Vector.Zero);
    public Avalonia.Vector PointerWorldPosition
    {
        get => GetValue(PointerWorldPositionProperty);
        set => SetValue(PointerWorldPositionProperty, value);
    }

    public static readonly StyledProperty<double> SelectionLeftProperty = AvaloniaProperty.Register<WorldCanvas, double>(nameof(SelectionLeft), defaultValue: 0.0);
    ...
    public static readonly StyledProperty<double> SelectionTopProperty = AvaloniaProperty.Register<WorldCanvas, double>(nameof(SelectionTop), defaultValue: 0.0);
    ...
    public static readonly StyledProperty<double> SelectionWidthProperty = AvaloniaProperty.Register<WorldCanvas, double>(nameof(SelectionWidth), defaultValue: 0.0);
    ...
    public static readonly StyledProperty<double> SelectionHeightProperty = AvaloniaProperty.Register<WorldCanvas, double>(nameof(SelectionHeight), defaultValue: 0.0);
    ...
    {
        _world = world;
        _worldObjRenderer = new WorldObjectRenderer(this);
        AddRenderer(_worldObjRenderer);
        AddCanvasInputHandler(new WorldCanvasInputHandler(this));
    }
    ...
    protected override void OnGLInitialized(GlInterface gl)
    {
        ...
        WorkspaceViewModel.Current.Workspace.WorldChanged += OnWorldChanged;
    }
    ...
    protected override void OnGLRenderColor(GlInterface gl)
    {
        ...
        if (_viewDirty)
        {
            _worldObjRenderer.OnViewChanged(viewRect);
            _viewDirty = false;
        }
    ...
    {
        string oldGroup = world.Group;
        RemoveRenderer(_worldObjRenderer);
        _world = world;
        _world.OnInitialized();
        _worldObjRenderer = new WorldObjectRenderer(this);
        AddRenderer(_worldObjRenderer);
        if (_world.Group != oldGroup)
        {
    ...
    WorldObjectRenderer _worldObjRenderer;

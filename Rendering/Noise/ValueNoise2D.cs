...
public class ValueNoiseParameters : INoiseParameters
{
    [EditEnum]
    public NoiseHelper.ValueAlgorithms Algorithm { get; set; } = NoiseHelper.ValueAlgorithms.Value;

    public void CopyFrom(INoiseParameters other)
    {
        if (other is ValueNoiseParameters args)
        {
    ...
    {
        return other is ValueNoiseParameters args && Algorithm == args.Algorithm;
    }
}
...
public class ValueNoise2D : INoise2D
{
    ...
    public ValueNoiseParameters Arguments { get; private set; }

    public ValueNoise2D(ValueNoiseParameters args)
    {
        Arguments = args;
    }

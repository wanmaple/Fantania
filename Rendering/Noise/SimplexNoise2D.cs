...
public class SimplexNoiseParameters : INoiseParameters
{
    [EditEnum]
    public NoiseHelper.SimplexAlgorithms Algorithm { get; set; } = NoiseHelper.SimplexAlgorithms.OrdinarySimplex;

    public bool Equals(INoiseParameters? other)
    {
        return other is SimplexNoiseParameters args && Algorithm == args.Algorithm;
    }

    public void CopyFrom(INoiseParameters other)
    {
        if (other is SimplexNoiseParameters args)
        {
...
public class SimplexNoise2D : INoise2D
{
    ...
    public SimplexNoiseParameters Arguments { get; private set; }

    public SimplexNoise2D(SimplexNoiseParameters args)
    {
        Arguments = args;
    }

...
public class LevelObjectRenderer : IRenderer
{
    ...
    public LevelObjectRenderer(LevelCanvas canvas)
    {
        _canvas = canvas;
    }
    ...
    {
        _canvas.Level.ObjectAdded += OnLevelObjectAdded;
        _canvas.Level.ObjectRemoved += OnLevelObjectRemoved;
        var viewRect = _canvas.GetViewRect();
        foreach (var obj in _canvas.Level.AllObjects)
        {
    ...
    void OnObjectRenderInfoChanged(LevelObject obj)
    {
        UpdateRenderable(obj);
    }

    void OnObjectVisibilityChanged(LevelObject obj)
    {
        if (obj.IsVisible)
        {
            Rect viewRect = _canvas.GetViewRect();
    ...
    void UpdateRenderable(LevelObject obj)
    {
    ...
    {
        foreach (LevelObject obj in _removing)
        {
            if (!_visibleObjects.Remove(obj)) continue;
            if (HideLevelObject(obj))
            {
                obj.RenderInfoChanged -= OnObjectRenderInfoChanged;
                obj.VisibilityChanged -= OnObjectVisibilityChanged;
            }
        ...
        _removing.Clear();
        foreach (LevelObject obj in _adding)
        {
            obj.OnEnterCanvas(gl);
    ...
    void ShowLevelObject(LevelObject obj)
    {
    ...
    bool HideLevelObject(LevelObject obj)
    {
        var batches = _batched[obj.Template];
        foreach (var batch in batches)
        {
    ...
    HashSet<LevelObject> _visibleObjects = new HashSet<LevelObject>(1024);
    Dictionary<DrawTemplate, List<BatchedRenderables>> _batched = new Dictionary<DrawTemplate, List<BatchedRenderables>>(16);

    HashSet<LevelObject> _adding = new HashSet<LevelObject>(10);
    HashSet<LevelObject> _removing = new HashSet<LevelObject>(10);

...
function ExportSettings:gamedataVariants(data)
    ...
    local function writeDataHeader(file, placementName, fields)
        file:write('/// This file is generated by Fantania, do not edit it manually.\n')
        file:write('#pragma once\n\n')
        file:write('#include "core/core.h"\n')
        file:write('#include "game/GameMacroes.h"\n')
        file:write('#include "game/field_types/FieldTypes.h"\n')
        file:write('#include <godot_cpp/godot.hpp>\n')
        file:write('#include <godot_cpp/classes/file_access.hpp>\n')
        file:write('\n')
        file:write('NS_GAME_BEGIN\n\n')
        file:write('struct ' .. placementName .. ' {\n')
        file:write('public:\n')
        file:write('\tbool read(const godot::Ref<godot::FileAccess> &fs);\n\n')
        for _, field in ipairs(fields) do
            local typeStr = ExportHelper.fieldType2CppFieldType(field.type)
            local retStr = ExportHelper.fieldType2CppGetterType(field.type)
            if not typeStr or not retStr then
            ...
            file:write(string.format('\tGETTER(%s, %s, %s);\n', fieldName, retStr, field.name))
        end
        file:write('};\n\n')
        file:write('NS_GAME_ENDED')
        file:flush()
        file:close()
    end

    local function writeDataCpp(file, placementName, fields)
        file:write('/// This file is generated by Fantania, do not edit it manually.\n')
        file:write('#include "game/miscs/TemplateSpecializations.h"\n')
        file:write('#include "game/generated/ExportedStrings.h"\n')
        file:write('#include "game/miscs/ResourceHelper.h"\n')
        file:write('#include "game/generated/' .. placementName .. '.h"\n\n')
        file:write('using namespace core;\n\n')
        file:write('NS_GAME_BEGIN\n\n')
        file:write('bool ' .. placementName .. '::read(const godot::Ref<godot::FileAccess> &fs) {\n')
        for _, field in ipairs(fields) do
            local typeStr = ExportHelper.fieldType2CppFieldType(field.type)
            local retStr = ExportHelper.fieldType2CppGetterType(field.type)
            if not typeStr or not retStr then
            ...
            local varName = string.lower(string.sub(field.name, 1, 1)) .. string.sub(field.name, 2)
            local fieldName = "_" .. varName
            local sameStr = typeStr == retStr
            if not sameStr then
                file:write(string.format('\t%s %s;\n', typeStr, varName))
                file:write(string.format('\tif (!StorageHelper::readBinary(fs, %s)) return false;\n', varName))
            else
                file:write(string.format('\tif (!StorageHelper::readBinary(fs, %s)) return false;\n', fieldName))
            end
            if not sameStr then
                if field.type == FieldTypes.String then
                    file:write(string.format('\t%s = ExportedStrings::AllStrings[%s];\n', fieldName, varName))
                elseif field.type == FieldTypes.Texture then
                    file:write(string.format('\t%s.texturePath = ResourceHelper::getTexturePath(%s);\n', fieldName, varName))
                    file:write(string.format('\t%s.uvRect = ResourceHelper::getUVRect(%s);\n', fieldName, varName))
                elseif field.type == FieldTypes.StringArray then
                    file:write(string.format('\t%s.resize(%s.num());\n', fieldName, varName))
                    file:write(string.format('\tfor (u32 i = 0; i < %s.num(); i++) {\n', varName))
                    file:write(string.format('\t\t%s[i] = ExportedStrings::AllStrings[%s[i]];\n', fieldName, varName))
                    file:write('\t}\n')
                elseif field.type == FieldTypes.TextureArray then
                    file:write(string.format('\t%s.resize(%s.num());\n', fieldName, varName))
                    file:write(string.format('\tfor (u32 i = 0; i < %s.num(); i++) {\n', varName))
                    file:write("\t\tTextureInfo info;\n")
                    file:write(string.format('\t\tinfo.texturePath = ResourceHelper::getTexturePath(%s[i]);\n', varName))
                    file:write(string.format('\t\tinfo.uvRect = ResourceHelper::getUVRect(%s[i]);\n', varName))
                    file:write(string.format('\t\t%s[i] = info;\n', fieldName))
                    file:write('\t}\n')
                else
                    Workspace.ThrowException("Unsupported field type for conversion: " .. tostring(field.type))
                end
            end
        end
    ...
    for _, structure in ipairs(placementStructures) do
        ...
        writeDataHeader(fStructHeader, placementName, structure.fields)
        local fStructCpp = io.open(structCppFile, "w")
        writeDataCpp(fStructCpp, placementName, structure.fields)
    ...
    for _, structure in ipairs(gamedataStructures) do
        ...
        writeDataHeader(fDataHeader, dataName, structure.fields)
        local fDataCpp = io.open(dataCppFile, "w")
        Workspace.Log("Generating " .. dataCppFile)
        writeDataCpp(fDataCpp, dataName, structure.fields)
    end

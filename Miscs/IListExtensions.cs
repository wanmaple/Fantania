...
public static class IListExtensions
{
    ...
    public static void MergeSort<T>(this IList<T> self, IComparer<T> comparer = null)
    {
        if (comparer == null)
        {
            comparer = Comparer<T>.Default;
        }
        MergeSort(self, 0, self.Count - 1, comparer);
    }

    private static void MergeSort<T>(IList<T> input, int p, int r, IComparer<T> comparer)
    {
        if (p < r)
        {
            int q = (p + r) / 2;
            MergeSort(input, p, q, comparer);
            MergeSort(input, q + 1, r, comparer);

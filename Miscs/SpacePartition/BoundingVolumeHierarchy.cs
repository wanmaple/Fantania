...
public class BoundingVolumeHierarchy<T> where T : IBVHItem
{
    ...
    public Rect Bounds => _root == null ? new Rect() : _root.bounds;
    ...
    BoundingVolumeHierarchyNode<T> InsertNode(BoundingVolumeHierarchyNode<T>? root, BoundingVolumeHierarchyNode<T> newNode)
    {
        ...
        Rect mergedLeft = root.left.bounds.Merge(newNode.bounds);
        Rect mergedRight = root.right.bounds.Merge(newNode.bounds);
        double incLeft = mergedLeft.Area() - root.left.bounds.Area();
        double incRight = mergedRight.Area() - root.right.bounds.Area();
        if (incLeft < incRight)
        {
    ...
    void BalanceNode(BoundingVolumeHierarchyNode<T> node)
    {
        ...
        if (balanceFactor > 1)
        {
            ...
            {
                double areaBefore = left.bounds.Area() + right.bounds.Area();
                Rect boundsRightAfter = rl.bounds.Merge(left.bounds);
                double areaAfter = rr.bounds.Area() + boundsRightAfter.Area();
            ...
            {
                double areaBefore = left.bounds.Area() + right.bounds.Area();
                Rect boundsRightAfter = rr.bounds.Merge(left.bounds);
                double areaAfter = rl.bounds.Area() + boundsRightAfter.Area();
                if (areaAfter <= areaBefore)
                {
                    right.left = left;
                    left.parent = right;
        ...
        else if (balanceFactor < -1)
        {
            ...
            {
                double areaBefore = left.bounds.Area() + right.bounds.Area();
                Rect boundsLeftAfter = lr.bounds.Merge(right.bounds);
                double areaAfter = ll.bounds.Area() + boundsLeftAfter.Area();
                if (areaAfter <= areaBefore)
                {
                    left.left = right;
                    right.parent = left;
            ...
            {
                double areaBefore = left.bounds.Area() + right.bounds.Area();
                Rect boundsLeftAfter = ll.bounds.Merge(right.bounds);
                double areaAfter = lr.bounds.Area() + boundsLeftAfter.Area();
                if (areaAfter <= areaBefore)
                {
                    left.right = right;
                    right.parent = left;
    ...
    BoundingVolumeHierarchyNode<T> CreateNewNode(Rect bounds, T item)
    {
        var node = ReuseNode();
        node.bounds = bounds;
        node.item = item;
    ...
    public void PointTest(Point point, IList<T> results, Predicate<T> filter = null)
    {
        InnerPointTest(_root, point, filter, results);
    }

    public void RectTest(Rect rect, ISet<T> results, Predicate<T> filter = null)
    {
        InnerRectTest(_root, rect, filter, results);
    }

    void InnerPointTest(BoundingVolumeHierarchyNode<T> node, Point point, Predicate<T> filter, IList<T> ret)
    {
        if (node == null) return;
    ...
    void InnerRectTest(BoundingVolumeHierarchyNode<T> node, Rect rect, Predicate<T> filter, ISet<T> ret)
    {
        if (node == null) return;

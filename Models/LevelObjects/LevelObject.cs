...
public abstract class LevelObject : ObservableObject, IBVHItem, IRenderable
{
    public event Action<LevelObject> TransformChanged;
    public event Action<LevelObject, RenderLayers, RenderLayers> LayerChanged;
    public event Action<LevelObject> RenderInfoChanged;
    public event Action<LevelObject, bool> SelectStateChanged;
    ...
    protected LevelObject(Workspace workspace)
    {
        do
    ...
    /// </summary>
    public virtual void OnReadyAdding(Level lv) { }
    /// <summary>
    /// 从Placements列表中选中后鼠标移出Canvas时触发
    /// </summary>
    public virtual void OnCancelAdding(Level lv) { }
    /// <summary>
    /// 从Placements列表中选中后鼠标点击放置时触发
    /// </summary>
    public virtual void OnPlaceFromAdding(Level lv) { }
    /// <summary>
    /// 进入BVH时触发
    /// </summary>
    public virtual void OnEnter(Level lv)
    {
        _template.RenderLayerChanged += OnTemplateLayerChanged;
    }
    /// <summary>
    /// 移出BVH时触发
    /// </summary>
    public virtual void OnExit(Level lv)
    {
        _template.RenderLayerChanged -= OnTemplateLayerChanged;
    }

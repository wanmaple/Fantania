...
public class World : ObservableObject
{
    ...
    public event Action<WorldObject> ObjectAdded;
    public event Action<WorldObject> ObjectRemoved;
    public event Action RenderInitialized;

    public IEnumerable<WorldObject> AllObjects => _allObjects;
    public WorldEnvironment Environment => _envWorld;
    public IEnumerable<WorldObject> SelectedObjects => _selections.Keys;
    public Rect Bounds => _bvh.Bounds;
    public WorldObject AddingObject => _addingObj;
    ...
    public World(string name, string group)
    {
        _name = name;
        _group = group;
    }
        ...
        await Task.Run(() =>
        {
            using (var fs = new FileStream(path, FileMode.Open, FileAccess.Read))
            {
                using (var br = new BinaryReader(fs))
                {
                    ...
                    {
                        WorldObject obj = serializer.Deserialize(br, workspace) as WorldObject;
                        AddObject(obj);
                    }
                }
            }
    ...
    public async Task Save(Workspace workspace)
    {
        ...
        await Task.Run(() =>
        {
            string path = Path.Combine(workspace.WorldsFolder, Name + ".bin");
    ...
    public void AddObject(WorldObject obj, bool placed = true)
    {
    ...
    public void ReadyAdd(WorldObject adding)
    {
        _addingObj = adding;
    ...
    public IReadOnlyList<WorldObject> PointTest(Point point, Predicate<WorldObject> filter = null)
    {
        var ret = new List<WorldObject>(16);
    ...
    public void PointTest(Point point, IList<WorldObject> list, Predicate<WorldObject> filter = null)
    {
    ...
    public ISet<WorldObject> RectTest(Rect rect, Predicate<WorldObject> filter = null)
    {
        var ret = new HashSet<WorldObject>(16);
        _bvh.RectTest(rect, ret, filter);
        return ret;
    }

    public void RectTest(Rect rect, ISet<WorldObject> set, Predicate<WorldObject> filter = null)
    {
    ...
    void OnObjectTransformChanged(WorldObject obj)
    {
        _bvh.UpdateItem(obj);
    }

    void OnObjectSelectStateChanged(WorldObject obj, bool selected)
    {
    ...
    internal BoundingVolumeHierarchy<WorldObject> _bvh = new BoundingVolumeHierarchy<WorldObject>();
    internal WorldObjects _allObjects = new WorldObjects();
    WorldEnvironment _envWorld = new WorldEnvironment();
    WorldObject _addingObj;
    Dictionary<WorldObject, Selection> _selections = new Dictionary<WorldObject, Selection>(16);
    bool _dirty = false;
}